...
export const getAccessToken = () => {
  ...
  if (
    ...
    return {
      accessToken: LOCALSTORAGE_VALUES.accessToken,
      refreshToken: LOCALSTORAGE_VALUES.refreshToken,
      expireTime: LOCALSTORAGE_VALUES.expireTime,
      timestamp: LOCALSTORAGE_VALUES.timestamp
    };
  }
  ...
  if (queryParams[LOCALSTORAGE_KEYS.accessToken]) {
    ...
    return {
      accessToken: queryParams[LOCALSTORAGE_KEYS.accessToken],
      refreshToken: queryParams[LOCALSTORAGE_KEYS.refreshToken],
      expireTime: queryParams[LOCALSTORAGE_KEYS.expireTime],
      timestamp: Date.now()
    };
  }
...
export const getClientAccessToken = async () => {
  if (
    ...
    console.log("Failed to get client access token from local storage!");
 ...
 */
export const hasTokenExpired = (givenToken = null) => {
  const { accessToken, timestamp, expireTime } =
    givenToken || LOCALSTORAGE_VALUES;
 ...
 * @returns {Promise}
 */
export const getCurrentUserProfile = async (givenToken = null) => {
  const token = givenToken || getAccessToken();
  if (!token) {
 ...
 * @returns {Promise}
 */
export const getCurrentlyPlaying = (givenToken = null) => {
  let token = givenToken || getAccessToken();
  if (token) {
 ...
 * @returns {Promise}
 */
export const getRecentlyPlayed = (givenToken = null, limit = 50) => {
  let token = givenToken || getAccessToken();
  if (token) {
 ...
 * @returns {Promise}
 */
export const getTrack = async (id, givenToken = null) => {
  let token = givenToken || (await getClientAccessToken());
  ...
  // If the client token exists, get the track
  if (token) {
 ...
 * @returns {Promise}
 */
export const searchTracks = async (query, givenToken = null, limit = 50) => {
  let token = null;

  console.log("Searching...");
  console.log(givenToken);

  // token = getAccessToken() || (await getClientAccessToken());

  token = givenToken || (await getClientAccessToken());
